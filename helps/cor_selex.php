...
<form action="" method="post">
  ...
  <div  class="qwe">
    ...
    <option value="Игрок">Игрока</option>
    <option value="Вип">Випа</option>
    <option value="Премиум">Премиума</option>
    <option value="Креатив">Креатива</option>
    <option value="Модер">Модера</option>
    <option value="Админ">Админа</option>
    <option value="Лорд">Лорда</option>
    <option value="Гл.Админ">Гл.Админа</option>
    <option value="Оператор">Оператора</option>
    <option value="Создатель">Создателя</option>
    <option value="Основатель">Основателя</option>
    <option value="Владелец">Владельца</option>
    <option value="Анти-Грифер">АГ</option>
    <option value="Бог">Бога</option>
    <option value="Большой Босс">Большого Босса</option>
    ...
    <option value="Основатель Сервера">Основателя (СД)</option>
    <option value="СоВладелец">СоВладельца</option>
    <option value="Владелец Сервера">Владельца(СД)</option>
  ...
  <form action="" method="post">
	...
	$mysqli->query ("SET NAMES 'utf8'");
      ...
      function printNickCor($result_set) {
     	 while (($row = $result_set->fetch_assoc()) !=false) {
           if ($_GET['go_redexcl'] == 'Vampic') {
             echo '<option value="'.$row['nickname'].'">'.$row['nickname'].' Seller - '.$row['seller'].'</option>';
           } else {
    ...
    if ($_GET['go_redexcl'] == 'Vampic') {
   		$result_set = $mysqli->query ("SELECT * FROM `buyers` WHERE `seller` IS NOT NULL");
  	} else {
 ...
 <form action="" method="post">
	...
	$mysqli->query ("SET NAMES 'utf8'");
      ...
      function printNickDel($result_set) {
     	 while (($row = $result_set->fetch_assoc()) !=false) {
           if ($_GET['go_redexcl'] == 'Vampic') {
           	 echo '<option value="'.$row['nickname'].'">'.$row['nickname'].' : '.$row['donate'].' Seller - '.$row['seller'].'</option>';
           } else {
...
echo "<br><br><br><br><br><br><br><hr><br></div>";
  ...
  function printYourBuyers ($get_buyers) {
      ...
      if ($_GET['go_redexcl'] == 'Vampic') {
      	echo  "<li><a href='".$row['vk_link']."'>".$row['vk_name']."</a> - ".$row['nickname']."<b><span style='color: red;'>『</span></b><i><span style='color: #3333CC'>".$row['donate']."</span></i><b><span style='color: red;'>』</span> Seller - <span style='color: #cd6133'>".$row['seller']."</span></b></li>";
      } else {
  ...
  if ($_GET['go_redexcl'] == 'Vampic') {
    $get_buyers = $mysqli->query ("SELECT * FROM `buyers` WHERE `seller` IS NOT NULL");
  } else {
